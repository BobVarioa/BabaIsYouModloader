function addunit(id, undoing_, levelstart_)
    ...
    if generaldata.flags[LOGGING] then
        if levelstart then
            dolog("init_object", "event",
                unit.strings[UNITNAME] .. ":" .. tostring(unit.values[XPOS]) .. ":" .. tostring(unit.values[YPOS]))
        elseif (undoing == false) then
            dolog("new_object", "event",
                unit.strings[UNITNAME] .. ":" .. tostring(unit.values[XPOS]) .. ":" .. tostring(unit.values[YPOS]))
        end
    end
end
...
function createall(matdata, x_, y_, id_, dolevels_, leveldata_)
    ...
    if (matdata[1] == "level") and dolevels then
        ...
        if (blocked["all"] == nil) and ((matdata[2] == nil) or testcond(matdata[2], 1)) then
            for b, unit in pairs(objectlist) do
                if (findnoun(b, nlist.brief) == false) and (b ~= "empty") and (b ~= "level") and
                    (blocked[target] == nil) then
...
function setunitmap()
    ...
    if (generaldata.strings[WORLD] == generaldata.strings[BASEWORLD]) and
        ((generaldata.strings[CURRLEVEL] == "89level") or (generaldata.strings[CURRLEVEL] == "33level")) then
        limit = 3
    end

    if (generaldata.strings[WORLD] == "baba_m") and
        ((generaldata.strings[CURRLEVEL] == "89level") or (generaldata.strings[CURRLEVEL] == "33level")) then
        limit = 2
    end
...
function poscorrect(unitid, rotation, zoom, offset)
    ...
    if (spritedata.values[VISION] == 0) or (unit.values[ZLAYER] >= 21) then
        if (unit ~= nil) then
